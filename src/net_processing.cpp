...
static void MarkBlockAsNotInFlight(const uint256& hash, NodeStateAccessor& state, bool clearState = true) {
    ...
    while (range.first != range.second) {
        ...
        if (itInFlight->second.first == state->m_id) {
            if (clearState)
                ClearDownloadState(itInFlight->second, state);
...
void ProcessBlockAvailability(NodeStateAccessor& state) {
...
void UpdateBlockAvailability(NodeStateAccessor& state, const uint256 &hash) {
...
void FindNextBlocksToDownload(NodeStateAccessor& state, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) {
    if (count == 0)
        return;
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        ...
        for (const CBlockIndex* pindex : vToFetch) {
            ...
            if (!state->fHaveWitness && IsWitnessEnabled(pindex->pprev, consensusParams)) {
            ...
            if (pindex->nStatus & BLOCK_HAVE_DATA || chainActive.Contains(pindex)) {
                ...
                if (pindex->nHeight > nWindowEnd) {
                    ...
                    if (vBlocks.size() == 0 && waitingfor != state->m_id) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc, std::map<NodeId, int>& other_node_ban_map)
{
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (stateDummy.IsInvalid(nDos) && nDos > 0)
                        {
                            ...
                            // (but dont take two NodeStateAccessors at once)
                            other_node_ban_map[fromPeer] += nDos;
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        assert(pindex);
        UpdateBlockAvailability(nodestate, pindex->GetBlockHash());
        ...
        if (fProcessBLOCKTXN)
            return ProcessMessage(pfrom, NetMsgType::BLOCKTXN, blockTxnMsg, nTimeReceived, chainparams, connman, interruptMsgProc, other_node_ban_map);

        if (fRevertToHeaderProcessing)
            return ProcessMessage(pfrom, NetMsgType::HEADERS, vHeadersMsg, nTimeReceived, chainparams, connman, interruptMsgProc, other_node_ban_map);
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (mapBlockIndex.find(headers[0].hashPrevBlock) == mapBlockIndex.end() && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            UpdateBlockAvailability(nodestate, headers.back().GetHash());
        ...
        assert(pindexLast);
        UpdateBlockAvailability(nodestate, pindexLast->GetBlockHash());
...
bool ProcessMessages(CNode* pfrom, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    // Process message
    bool fRet = false;
    std::map<NodeId, int> other_node_ban_map;
    try
    {
        fRet = ProcessMessage(pfrom, strCommand, vRecv, msg.nTime, chainparams, connman, interruptMsgProc, other_node_ban_map);
        if (interruptMsgProc)
            return false;
        if (!pfrom->vRecvGetData.empty())
            fMoreWork = true;
    ...
    LOCK(cs_main);
    for (auto& entry : other_node_ban_map) {
        NodeStateAccessor nodestate = State(entry.first);
