...
public:
    ...
    CNodeStateAccessor(CNodeStateAccessor&& o) {
        // using std::move ensures o.pstate is set to null,
        // avoiding the unlock in our destructor
        pstate = std::move(o.pstate);
    }

    explicit operator bool() const { return (bool)pstate; }

    CNodeState* operator->() { assert(CNodeStateAccessor::operator bool()); return &(*pstate); }
    const CNodeState* operator->() const {  assert(CNodeStateAccessor::operator bool());; return &(*pstate); }
};
...
void InitializeNode(CNode *pnode, CConnman& connman) {
    nodeStateStorage.AddStateForNode(pnode->GetId(), pnode->addr, pnode->GetAddrName());
    if(!pnode->fInbound)
...
void MaybeSetPeerAsAnnouncingHeaderAndIDs(NodeId nodeid, CConnman& connman) {
    AssertLockHeld(cs_main);
    CNodeStateAccessor nodestate = State(nodeid);
...
void AnnounceBlocksToPeer(CNode* pto, const std::vector<uint256>& vBlockHashesToAnnounce, CConnman& connman) {
    //
    ...
    bool fRevertToInv = ((!state->fPreferHeaders &&
                         (!state->fPreferHeaderAndIDs || vBlockHashesToAnnounce.size() > 1)) ||
                        vBlockHashesToAnnounce.size() > MAX_BLOCKS_TO_ANNOUNCE);
    ...
    if (!fRevertToInv) {
        ...
        BOOST_FOREACH(const uint256 &hash, vBlockHashesToAnnounce) {
    ...
    if (!fRevertToInv && !vHeaders.empty()) {
        if (vHeaders.size() == 1 && state->fPreferHeaderAndIDs) {
            ...
            if (!fGotBlockFromCache) {
                CBlock block;
                bool ret = ReadBlockFromDisk(block, pBestIndex, Params().GetConsensus());
                assert(ret);
    ...
    if (fRevertToInv) {
        ...
        // in the past.
        if (!vBlockHashesToAnnounce.empty()) {
            const uint256 &hashToAnnounce = vBlockHashesToAnnounce.back();
...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
    ...
    connman->ForEachNode([this, &pcmpctblock, pindex, &msgMaker, fWitnessEnabled, &hashBlock](CNode* pnode) {
        ...
        CNodeStateAccessor state = State(pnode->GetId());
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        {
        LOCK(cs_main);
        CNodeStateAccessor state = State(pfrom->GetId());
